...
author: Ian Lintner
image: /images/modern-oauth2-k8s-social.png
...
This post explores how to run a modern, production-ready OAuth2 Authorization Server in Kubernetes, leveraging **Spring Boot 3.2** and **Spring Authorization Server**[^sas], and how to integrate it with the cloud-native ecosystem like **Istio**[^istio] and **OAuth2 Proxy**[^oauth2proxy].
...
sequenceDiagram
    ...
    SPA-->>Auth: /authorize + code_challenge (S256)
    Auth-->>User: Login + consent
    User-->>Auth: Credentials + consent
    Auth-->>SPA: 302 with code
    SPA-->>Auth: /token + code_verifier
    Auth-->>SPA: ID token + access token (JWT)
    SPA-->>API: GET /me Authorization: Bearer
    API-->>Auth: Validate JWT (cached JWKS)
    Auth-->>API: Signature + claims OK
...
```mermaid
sequenceDiagram
    participant Job as Cron / Service A
    participant Auth as Auth Server
    participant SvcB as Service B (resource)
    Job-->>Auth: POST /oauth2/token (client_credentials)
    Auth-->>Job: access_token (aud=ServiceB)
    Job-->>SvcB: Call API with Bearer token
    SvcB-->>Auth: Introspect / validate JWT (if opaque)
    Auth-->>SvcB: Active=true, scopes
    SvcB-->>Job: 200 OK
```

### OIDC SSO via GitHub + OAuth2 Proxy
...
sequenceDiagram
    ...
    User->>Proxy: GET https://app.example.com
    Proxy-->>User: 302 to Auth (OIDC)
    User->>Auth: /authorize
    Auth-->>GH: Redirect to GitHub
    User->>GH: Approve app
    GH-->>Auth: Authorization code
    Auth-->>Proxy: Tokens + ID token
    Proxy-->>User: Set session cookie, forward to app
```
...
```mermaid
flowchart LR
    Client --> Gateway[Istio Ingress Gateway]
    Gateway -->|validates JWT| RequestAuth[RequestAuthentication]
    RequestAuth --> AuthzPolicy[AuthorizationPolicy]
    AuthzPolicy --> Workload[my-service pod]
    Workload -->|introspect/metadata| AuthServer[Spring Auth Server]
```
...
- **PKCE (Proof Key for Code Exchange)**: Mandatory for all public clients (SPAs, Mobile). Enforce `code_challenge_method=S256` and reject plain challenges[^pkce].
...
- **Key Management**: Rotate signing keys quarterly via the JWKS endpoint[^jwks]. Publishes happen through `/actuator/refresh` + External Secrets so that rotations don't require restarts.
...
[^rfc6749]: IETF RFC 6749: The OAuth 2.0 Authorization Framework — https://datatracker.ietf.org/doc/html/rfc6749

[^pkce]: IETF RFC 7636: Proof Key for Code Exchange (PKCE) — https://datatracker.ietf.org/doc/html/rfc7636

[^sas]: Spring Authorization Server Reference — https://docs.spring.io/spring-authorization-server/reference/

[^oauth2proxy]: OAuth2 Proxy Docs — https://oauth2-proxy.github.io/oauth2-proxy/

[^istio]: Istio RequestAuthentication / AuthorizationPolicy — https://istio.io/latest/docs/tasks/security/authorization/

[^jwks]: OpenID Connect Discovery (JWKS endpoint) — https://openid.net/specs/openid-connect-discovery-1_0.html
