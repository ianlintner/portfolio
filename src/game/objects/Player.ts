...
export class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(scene: Scene, x: number, y: number) {
        ...
        if (scene.input.keyboard) {
            ...
            this.shootKey = scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.F);
        }
    }
    ...
    update() {
        ...
        // Movement
        if (this.cursors.left.isDown || this.wasd.left.isDown || this.wasd.a.isDown) {
            this.setVelocityX(-speed);
            this.setFlipX(true);
            this.anims.play('walk', true);
        } else if (this.cursors.right.isDown || this.wasd.right.isDown || this.wasd.d.isDown) {
            this.setVelocityX(speed);
            this.setFlipX(false);
            this.anims.play('walk', true);
        } else {
        ...
        if ((this.cursors.up.isDown || this.wasd.up.isDown || this.wasd.w.isDown || this.jumpKey.isDown) && isGrounded) {
    ...
    private shoot() {
        ...
        if (time > this.lastShotTime + 500) {
            ...
            this.scene.events.emit('player-shoot', this.x, this.y, this.flipX ? -1 : 1);
