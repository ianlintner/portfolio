...
export class BaseLevel extends Scene {
    ...
    create() {
        ...
        this.physics.add.overlap(this.hairballs, this.enemies, this.shootEnemy, undefined, this);
        this.physics.add.collider(this.hairballs, this.platforms, (ball) => ball.destroy());
    ...
    private collectItem(player: any, item: any) {
        ...
        if (item.texture.key === 'hairball') { // Reusing texture for catnip for now? No, let's use a different one or tint
    ...
    private hitEnemy(player: any, enemy: any) {
        ...
        if (player.body.velocity.y > 0 && player.y < enemy.y - enemy.body.height / 2) {
            // Kill enemy
            enemy.destroy();
