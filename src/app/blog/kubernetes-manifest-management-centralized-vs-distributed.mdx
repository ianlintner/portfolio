...
After years of operating Kubernetes platforms and observing patterns across organizations of all sizes, I've seen two dominant approaches emerge:
...
A fintech company I worked with adopted centralized manifests for their 40-service platform. Their reasoning:
...
graph TB
    subgraph "Platform Repo (Centralized)"
        ...
        ClusterConfig["Cluster Configuration<br/>• Namespaces<br/>• RBAC<br/>• Network Policies<br/>• Resource Quotas"]
        SharedInfra["Shared Infrastructure<br/>• Ingress Controllers<br/>• Service Mesh<br/>• Monitoring Stack<br/>• Cert Manager"]
        BaseTemplates["Base Templates<br/>• Deployment patterns<br/>• Security policies<br/>• Labeling standards"]
    ...
    subgraph "App Repos (Distributed)"
        ...
        K8sDir1["k8s/<br/>• Deployment<br/>• Service<br/>• HPA"]
        K8sDir2["k8s/<br/>• Deployment<br/>• ConfigMap"]
        K8sDir3["k8s/<br/>• Deployment<br/>• CronJob"]
...
## 🛣️ Sharing Paved Paths: Observability, Auth, & More
...
| **Platform Team Burden** | ⭐⭐⭐ High | ⭐ Low | ⭐⭐ Medium |
| **Consistency** | ⭐⭐⭐ High | ⭐ Low (without policies) | ⭐⭐⭐ High |
| **Audit Trail** | ⭐⭐⭐ Easy | ⭐ Distributed | ⭐⭐ Medium |
| **Cross-Cutting Changes** | ⭐⭐⭐ Easy | ⭐ Hard | ⭐⭐ Medium |
| **Service Independence** | ⭐ Low | ⭐⭐⭐ High | ⭐⭐⭐ High |
| Scaling (50+ services) | ⭐ Challenging | ⭐⭐⭐ Natural | ⭐⭐⭐ Designed for |
...
*   **Centralized Repo:**
    *   **Pros:** The agent can see the *entire* system state in one place. It can easily spot that "changing this Service name will break that Ingress." It's easier to build a Knowledge Base (KB) from a single repo.
...
### 🏗️ Blueprint: Building a "Cluster-Aware" Copilot Agent
...
*   **Kubernetes MCP Server:** Connect your agent to a read-only `ServiceAccount` in the cluster.
    *   *Tool:* `kubectl_get_resources` (allowed: Pods, Services, Ingress, Events)
    *   *Tool:* `kubectl_describe` (for debugging crash loops)
    *   *Tool:* `helm_list` (to see what's installed)
