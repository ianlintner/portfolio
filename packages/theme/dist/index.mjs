...
function useCookieStorage(key, initialValue, options = {}) {
  const [storedValue, setStoredValue] = useState4(initialValue);
  useEffect4(() => {
...
function useMediaQuery(query, defaultValue = false) {
  const [matches, setMatches] = useState5(defaultValue);
  useEffect5(() => {
    if (typeof window === "undefined") return;
...
function CodeLanguageProvider({ children, defaultLanguage = "typescript" }) {
  ...
  const getLabel = useCallback3((lang) => {
    return LANGUAGE_LABELS[lang] || lang;
  }, []);
  const getExtension = useCallback3((lang) => {
