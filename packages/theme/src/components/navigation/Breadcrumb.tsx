...
export interface BreadcrumbProps extends React.HTMLAttributes<OL> {
    ...
    <nav aria-label="Breadcrumb">
      ...
      >
        {items.map((item, idx) => {
          ...
          const content = item.href && !isLast && !item.active ? (
            <a
            ...
            <span className={cn(isLast || item.active ? "text-foreground" : undefined)}>
              {item.label}
            </span>
          );
          return (
